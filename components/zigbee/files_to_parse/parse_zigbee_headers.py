...
typedefs = [t[1].type for t in ast.children() if type(t[1]) == c_ast.Typedef]
enums = [t for t in typedefs if type(t.type) == c_ast.Enum]
...
def write_profileIDs(enums):
    enum = list(filter(lambda e: e.declname == 'esp_zb_ha_standard_devices_t', enums))[0]
...
with open('zigbee_const.py', 'w') as f:
    f.write('import esphome.codegen as cg\n')
    f.write(write_profileIDs(enums))
    f.write(write_clusterIDs(enums))
    f.write(write_clusterRoles(enums))
    f.write(write_ZBtypes(enums))
...
with open('c_helpers.c', 'w') as f:
    f.write(write_cluster_list_aou(enums))
    f.write(write_attr_list_create(enums))
    f.write(write_attr_add(enums))
